...
```powershell
# Project root path
$PROJECT = "c:\Users\ASUS\OneDrive\Desktop\COLLEGE EVERYTHING\hackathon\dssa vit\Vitalscore finance"

# Start databases
cd $PROJECT; docker-compose up -d

# Install ALL backend deps (run once)
$services = @("ai-categorization","auth-service","gamification-service","score-engine","transaction-ingestion","user-profile")
foreach ($svc in $services) { Set-Location "$PROJECT\backend\services\$svc"; npm install }
Set-Location $PROJECT

# Install frontend deps (run once)
cd "$PROJECT\frontend\web"; npm install; cd $PROJECT

# Start frontend (main terminal)
cd "$PROJECT\frontend\web"; npm run dev

# Start a backend service (open new terminal for each)
cd "$PROJECT\backend\services\score-engine"; npm run dev
```

---

*VitalScore Finance v4.0 | Vista HackArena 2025 | 🇮🇳 India-First | Free-Tier Verified*
